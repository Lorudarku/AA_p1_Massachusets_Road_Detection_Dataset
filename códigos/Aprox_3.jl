...
function tf(senal,Fs)
    ...
    m = Int(round(500*2*length(senalFrecuencia)/Fs));
    resultado=[]

    for i in 0:9
        push!(resultado,mean(senalFrecuencia[m*i+1:m*(i+1)]));
        push!(resultado,std(senalFrecuencia[m*i+1:m*(i+1)]));
    end
...
function sistemaRRNNAA(inputs,targets,topology,parada, maxMellora)
    ...
    e1=confusionMatrix(round.(ann(trainingIn))',trainingTar')
    push!(eTraining,e1);
    e2=confusionMatrix(round.(ann(testIn))',testTar')
    push!(eTest,e2);
    e3=confusionMatrix(round.(ann(validationIn))',validationTar')
    push!(eValidation,e3);
    best=deepcopy(ann);
    bestE=[e2;e1]
    while (e3[2]>parada) & (actual<maxMellora)
        Flux.train!(loss, params(ann), [(trainingIn, trainingTar)], ADAM(0.01));
        e1=confusionMatrix(round.(ann(trainingIn))',trainingTar')
        push!(eTraining,e1);
        e2=confusionMatrix(round.(ann(testIn))',testTar')
    ...
    [best,eTest,eTraining,eValidation,bestE]
end
...
RRNNAAaux=sistemaRRNNAA(caracteristicas[1],caracteristicas[2],[5 10],0.15,15)
