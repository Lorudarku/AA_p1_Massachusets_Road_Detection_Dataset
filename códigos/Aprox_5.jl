...
function estraccionCaracteristicas(ruta)
    inputs=[];
    targets=[]
    violin=readdir(ruta*"/vio");
    trompeta=readdir(ruta*"/tru");
    tam=length(violin)+length(trompeta);
    i=1;
    ant=-1;
    for c in violin
        push!(inputs,transformar(ruta*"/vio/"*c));
        push!(targets,"violin");
    ...
    for c in trompeta
        push!(inputs,transformar(ruta*"/tru/"*c));
        push!(targets,"trompeta");
        porcentaxe=floor((i/tam)*100);
        if (porcentaxe!=ant)
            println(porcentaxe,"%");
            ant=porcentaxe;
